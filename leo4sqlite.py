...
class InputDialogs(QWidget):
    ...
    def get_ext_db(self, c):
        ...
        # 'C:\Users\TSC\Desktop\leo-editor\leo\Icons\application-x-leo-outline.png'       
        options = QFileDialog.Options()
        options |= QFileDialog.DontUseNativeDialog
        QInputDialog.setStyleSheet(self, "padding: 3px");
        QInputDialog.setStyleSheet(self, "background: white");
        db_fname, _ = QFileDialog.getOpenFileName(self,"select an external database:", "","db files (*.db*)", options=options)
        if db_fname == '':
            g.es('cancel')
